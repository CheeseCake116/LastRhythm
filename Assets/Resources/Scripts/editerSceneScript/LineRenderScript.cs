...
public class LineRenderScript : MonoBehaviour
{
    ...
    // playScrollSpeed = 초당 이동하는 거리
    // yOffset = 플레이시간 * playScrollSpeed * -1 (노드가 아래로 이동하므로 음수값을 가짐)
    [HideInInspector] public float xOffset = 0, yOffset = 0, playScrollSpeed = 4f; 
    ...
    float nodeWidth = 1;
    float nodeHeight = 2; // zoom 1 -> nodeHeight 2
    float cellHeight;
    ...
    int beat = 4;
    float nodeOffset = 0f;
    public float zoom = 1; // zoom 1 -> playScrollSpeed 4
    float bpm = 120f;
    float musicSpeed = 1f;
    ...
    private void Start()
    {
        ...
        judgeObj.transform.position = new Vector3(-5, -4, -0.2f);
    ...
    {
        nodeList.Clear();
        List<NodeInfo> nodeBpmList = noteMngScript.nodeBpmList; // BPM이 수정된 노드의 리스트
        int preNode = 0, nextNode = 0, totalNode;
        
        if (mSource.clip == null) // 곡이 안 정해진 경우 노래 길이를 알 수 없으므로 종료
            return;

        float clipLength = mSource.clip.length; // 노래 길이

        // 각 노드에 저장되는 정보를 담는 변수
        float _bpm = bpm;
        float _yoffset = 0;
        ...
        {
            totalNode = (int)Mathf.Ceil(clipLength / (60f / bpm)); // 총 마디 갯수는 길이 / 한박자길이 + 1
            for (int n = 0; n < totalNode; n++)
            {
                // 각 노드 정보를 초기화하어 nodeList에 할당
    ...
    {
        int textCount = nodeNumbers.Count; // 한 화면에 보이는 텍스트 갯수
        float yos = yOffset + baseY + (nodeOffset * playScrollSpeed);
    ...
    *//*private void OnDrawGizmos()
    ...
    void DrawGrid()
    {
        ...
        float xos = xOffset + baseX;
        float yos = yOffset + baseY + nodeOffset * playScrollSpeed * zoom;

        node = (int)(winHeight / nodeHeight * zoom) + 3;

        if (nodeList.Count > 0)
        {
            float _nodeHeight = nodeHeight * zoom;
    ...
    void DrawNodeLine()
    {
        ...
        {
            float xos = xOffset + baseX;
            float yos = yOffset + baseY + (nodeOffset * playScrollSpeed * zoom) + nodeList[selectedNode].offset;
    ...
    void DrawSpectrum()
    {
        ...
        int width = 600;
        int height = (int)(mSource.clip.length * playScrollSpeed * zoom) * 100;
        int specStart = -200;
        int specEnd = 1100;
    ...
    IEnumerator CoroutSpectrum()
    {
        ...
        Debug.Log("전체 시간 : " + mSource.clip.length);
        scrollHeight = playScrollSpeed * zoom * mSource.clip.length;
        yOffsetLimit = -scrollHeight;
